...
mem_init(void)
{
	...
	// cprintf("mem_init: npages = %d\n", npages);
	// cprintf("mem_init: pages[] size = %d\n", npages * sizeof(struct PageInfo));
...
page_init(void)
{
	...
	// for (i = 0; i < npages; i++) {
	// 	pages[i].pp_ref = 0;
	// 	pages[i].pp_link = page_free_list; // head -> pages[npages-1] -> ...-> pages[1] -> pages[0] -> NULL
	// 	page_free_list = &pages[i];
	// }
	cprintf("page_init: npages = %d\n", npages);
	cprintf("page_init: PADDR((boot_alloc(0))) / PGSIZE = %d\n", PADDR((boot_alloc(0))) / PGSIZE);
	for (i = 0; i < npages; i++) {
		if (i == 0) { // page 0
			pages[i].pp_ref = 1;
			...
			pages[i].pp_ref = 1;
			pages[i].pp_link = NULL;
		} else if (i >= (size_t)EXTPHYSMEM / PGSIZE && i < PADDR((boot_alloc(0))) / PGSIZE) { // kernel and data structures
			pages[i].pp_ref = 1;
			pages[i].pp_link = NULL;
		} else {
			pages[i].pp_ref = 0;
...
page_alloc(int alloc_flags)
{
	...
	page_free_list = pp->pp_link; // 指向下一页
	pp->pp_link = NULL; // 避免double-free
